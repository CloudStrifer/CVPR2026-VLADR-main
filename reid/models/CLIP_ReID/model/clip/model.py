...
class DomainAdapter(nn.Module):
    """Small zero-initialized residual adapter for one continual domain."""

    def __init__(self, d_model: int, bottleneck_dim: int, scale: float = 1.0):
        super().__init__()
        if bottleneck_dim <= 0:
    ...
    def forward(self, x: torch.Tensor):
        ...
        output = self.up(
            self.activation(
                self.down(x.to(dtype=adapter_dtype))
            )
        )
        return output.to(dtype=input_dtype) * self.scale
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None):
        ...
        self._active_domain_adapter = None

    def add_domain_adapter(self, name, bottleneck_dim, scale=1.0):
        name = str(name)
        if not name or '.' in name:
            raise ValueError(
                'adapter name must be non-empty and cannot contain "."'
            )
        if name in self.domain_adapters:
            adapter = self.domain_adapters[name]
            if adapter.down.out_features != int(bottleneck_dim):
                raise ValueError(
                    'adapter {!r} already exists with bottleneck {}, got {}'
                    .format(
                        name,
            ...
            if not np.isclose(adapter.scale, float(scale)):
                raise ValueError(
                    'adapter {!r} already exists with scale {}, got {}'
                    .format(name, adapter.scale, scale)
                )
            return adapter

        adapter = DomainAdapter(
            self.ln_2.normalized_shape[0],
            int(bottleneck_dim),
            scale=scale,
        ).to(device=self.mlp.c_fc.weight.device)
        self.domain_adapters[name] = adapter
        return adapter

    def set_active_domain_adapter(self, name=None):
        if name is None:
            self._active_domain_adapter = None
            return
        name = str(name)
        if name not in self.domain_adapters:
            raise KeyError('unknown domain adapter: {!r}'.format(name))
        self._active_domain_adapter = name

    def get_active_domain_adapter(self):
        return self._active_domain_adapter
    ...
    def forward(self, x: torch.Tensor):
        ...
        residual = self.mlp(normalized)
        if self._active_domain_adapter is not None:
            residual = residual + self.domain_adapters[
                self._active_domain_adapter
            ](normalized)
        x = x + residual
        return x
...
class VisionTransformer(nn.Module):
    ...
    def add_domain_adapter(
        ...
        blocks = list(self.transformer.resblocks)
        if not 1 <= last_blocks <= len(blocks):
            raise ValueError(
                'adapter last_blocks must be in [1, {}], got {}'
                .format(len(blocks), last_blocks)
            )
        for block in blocks[-last_blocks:]:
            block.add_domain_adapter(
                name,
    ...
    def domain_adapter_names(self):
        ...
        for block in self.transformer.resblocks:
            names.update(block.domain_adapters.keys())
        return tuple(sorted(names))

    def set_active_domain_adapter(self, name=None):
        if name is not None:
            name = str(name)
            if name not in self.domain_adapter_names():
                raise KeyError('unknown domain adapter: {!r}'.format(name))
        for block in self.transformer.resblocks:
            block_name = (
                name if name in block.domain_adapters else None
            )
            block.set_active_domain_adapter(block_name)
        self._active_domain_adapter = name
    ...
    def domain_adapter_parameters(self, name):
        ...
        found = False
        for block in self.transformer.resblocks:
            if name in block.domain_adapters:
                found = True
                yield from block.domain_adapters[name].parameters()
        if not found:
            raise KeyError('unknown domain adapter: {!r}'.format(name))
